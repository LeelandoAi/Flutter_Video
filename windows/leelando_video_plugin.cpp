...
// static
void LeelandoVideoPlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows *registrar) {
  auto channel =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          registrar->messenger(), "leelando_video/fullscreen",
          &flutter::StandardMethodCodec::GetInstance());

  auto plugin = std::make_unique<LeelandoVideoPlugin>(registrar);
...
LeelandoVideoPlugin::LeelandoVideoPlugin(flutter::PluginRegistrarWindows* registrar) {
...
LeelandoVideoPlugin::~LeelandoVideoPlugin() {
  if (fullscreen_ && IsWindow(window_)) {
    SetFullscreen(false);
  }
}

void LeelandoVideoPlugin::HandleMethodCall(
...
bool LeelandoVideoPlugin::SetFullscreen(bool fullscreen) {
  if (!IsWindow(window_)) {
    return false;
  }
