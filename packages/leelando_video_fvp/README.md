...
`leelando_video_fvp` 是基于 FVP 和 libmdk 的 `leelando_video` 可插拔播放内核，支持 Android、iOS、Windows 和 macOS。

## 安装

```bash
flutter pub add leelando_video leelando_video_fvp
```
...
FVP 最终运行时租约释放前，`leelando_video` 会先释放适配器及其内部 `VideoPlayerController`；随后本包调用空 `platforms` 的 `fvp.registerWith`，恢复激活 FVP 前的 `VideoPlayerPlatform` 实例。
